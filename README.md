...
- [x] What problem does the context API help solve?
   ...
   - Cuts down on the prop drilling within the deepest layer of component and its children.

- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
  
  - **reducers** - specify how the application's state changes in response to actions sent to the store. 
  - **actions** - describes state changes -> Actions in Redux are packets of information that contain an action type and associated data (payload)
...
- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?
  - Application store -> all components can access the global state in a centralized location
  - Component -> (state lives within the component)
- [x]  Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
  - redux-thunk -> middleware that assigns async logic to interact with the redux store. Also a thunk is a function within a function. It calls the action creator in the parameter that defines dispatch and getState
- [x]  What is your favorite state management system you've learned and this sprint? Please explain why!
    - context, not as many moving parts, plus I honestly like the concept of prop drilling, but having a way to make it manageable is helpful.
